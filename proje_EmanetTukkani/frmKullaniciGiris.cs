	...
	public partial class frmKullaniciGiris : DevExpress.XtraEditors.XtraForm
	{
		...
		private void btnGiris_Click(object sender, EventArgs e)
		{
			try
			{
				...
				bool giris = false;
				using (SqlCommand cmd=new SqlCommand(@"Select KullaniciID, KullaniciAdSoyad From Kullanici Where KullaniciKodu=@KullaniciKodu AND KullaniciSifre=@KullaniciSifre AND Aktif=1", CsDosyası.csBaglanti.BaglantiGetir()))
				{
					cmd.Parameters.Add("@KullaniciKodu", SqlDbType.Int).Value = txtKullaniciKodu.Text;
					cmd.Parameters.Add("@KullaniciSifre", SqlDbType.Int).Value = txtKullaniciSifre.Text;
					using (SqlDataReader dr=cmd.ExecuteReader())
					{
						if (dr.Read())
						{
							giris = true;
							CsDosyası.csKullanici.KullaniciAdSoyad = dr["KullaniciAdSoyad"].ToString();
							CsDosyası.csKullanici.KullaniciID = dr["KullaniciID"].ToString();
						}
					}
				}
				if (giris)
